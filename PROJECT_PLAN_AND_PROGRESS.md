...
- [ ] Typography scale (fluid clamp() headings/body) defined in Tailwind config
- [ ] Color tokens (from actual brand, not the SRS PDF's Gamma theme) defined
- [ ] Base Blade components: `<x-container>`, `<x-button>`, `<x-section-heading>`, `<x-card>`
- [ ] Nav + footer components (with mobile overlay menu)
...
## 6. Content status
- [ ] SRS/FSD PDF content mapped into seeders/CMS — **done for placeholder text** (all homepage copy, services, pricing tiers, about copy in the PDF is usable as-is or as a first draft)
- [ ] PPT content — **pending upload**, will replace/extend placeholder copy once received
